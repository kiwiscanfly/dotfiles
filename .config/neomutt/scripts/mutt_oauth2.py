...
ap.add_argument('--client-secret-file', type=str, default='',
                help='Path to Google Cloud client_secret.json file')
...
# Load credentials from client_secret.json if provided
if args.client_secret_file:
    try:
        with open(args.client_secret_file, 'r') as f:
            creds = json.load(f)
            # Google credentials can be under 'installed' or 'web' key
            if 'installed' in creds:
                args.client_id = args.client_id or creds['installed']['client_id']
                args.client_secret = args.client_secret or creds['installed']['client_secret']
            elif 'web' in creds:
                args.client_id = args.client_id or creds['web']['client_id']
                args.client_secret = args.client_secret or creds['web']['client_secret']
            else:
                sys.exit('ERROR: client_secret.json file must have "installed" or "web" key')
    except FileNotFoundError:
        sys.exit(f'ERROR: client_secret file not found: {args.client_secret_file}')
    except json.JSONDecodeError:
        sys.exit(f'ERROR: client_secret file is not valid JSON: {args.client_secret_file}')
...
if not token:
    ...
    token['authflow'] = args.authflow or 'authcode'  # Default to authcode for CLI usage
