...
vec4 COLOR = vec4(0.83, 0.53, 0.61, 0.8); // Gruvbox pink #d3869b
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // don't clamp yet; we need to know if it's > 1.0 (finished)
     ...
     if (isModeChange > 0.0 && isAnimating > 0.0) {
        ...
        // float easedProgress = easeOutElastic(rippleProgress);
        float easedProgress = easeOutBounce(rippleProgress);
